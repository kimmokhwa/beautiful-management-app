...
export const ProcedureList: React.FC<{ showDownloadButton?: boolean }> = ({ showDownloadButton = true }) => {
  ...
  useEffect(() => {
    ...
    const customerPrice = editingProcedure.price || 0;
  ...
  }, [editingProcedure.cost, editingProcedure.price]);
  ...
  const handleRequestSort = (property: keyof Procedure) => {
    ...
    const sorted = [...filteredProcedures].sort((a, b) => {
      if (property === 'price' || property === 'cost' || property === 'margin' || property === 'margin_rate') {
          ...
          <TableBody>
            {filteredProcedures.map((procedure) => (
              <TableRow key={procedure.id}>
                ...
                <TableCell sx={{ textAlign: 'center' }} align="right">{(procedure.price || 0).toLocaleString()}원</TableCell>
                <TableCell sx={{ textAlign: 'center', color: '#2E7D32', fontWeight: 600 }}>
                  {procedure.cost?.toLocaleString()}원
                </TableCell>
                <TableCell sx={{ textAlign: 'center' }} align="right">{procedure.margin.toLocaleString()}원</TableCell>
                <TableCell sx={{ textAlign: 'center' }} align="right">{procedure.margin_rate}%</TableCell>
