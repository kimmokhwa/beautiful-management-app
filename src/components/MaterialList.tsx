  ...
  const handleSave = async () => {
    try {
      if (isEditing && editingMaterial.id) {
        await materialsApi.update(editingMaterial.id, editingMaterial);
      } else {
        await materialsApi.create(editingMaterial as Omit<ExtendedMaterial, 'id'>);
      }
      handleClose();
      loadMaterials();
    } catch (error) {
          ...
          <TableBody>
            {filteredMaterials.map((material) => (
              ...
              >
                ...
                <TableCell sx={{ textAlign: 'center', color: '#2E7D32', fontWeight: 600 }}>{material.cost.toLocaleString()}원</TableCell>
